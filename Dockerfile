...
RUN echo "gem 'figaro'" >> Gemfile && echo "gem 'daemons'" >> Gemfile && echo "gem 'delayed_job_active_record'" >> Gemfile && \
    bundle install && rails generate delayed_job:active_record && \
    sed -i "s/  config.available_locales = \[:en, :ca, :es\]/  config.available_locales = \[:en, :ca, :es, :fr\]/g" config/initializers/decidim.rb && \
